...
import numpy as np
import theano_lstm_2

from out_to_in_op import OutputFormToInputFormOp

from theano_lstm_2 import Embedding, LSTM, RNN, StackedCells, Layer, create_optimization_updates, masked_loss, MultiDropout
...
class Model(object):
    ...
    def setup_train(self):
        ...
        # apply dropout
        if self.dropout > 0:
            time_masks = theano_lstm_2.MultiDropout( [(num_time_parallel, shape) for shape in self.t_layer_sizes], self.dropout)
        else:
            time_masks = []
        ...
        # apply dropout
        if self.dropout > 0:
            pitch_masks = theano_lstm_2.MultiDropout( [(num_timebatch, shape) for shape in self.p_layer_sizes], self.dropout)
        else:
            pitch_masks = []
