...
class Gate(object):
    """
    Base object for LSTM variant 2 gate.

    A layer has an input set of neurons, and
    a hidden activation. The activation, f, is a
    function applied to the affine transformation
    of x by the connection matrix W.
    """

    def __init__(self, hidden_size, activation, clip_gradients=False):
        self.hidden_size = hidden_size
        self.activation  = activation
        self.clip_gradients = clip_gradients
        self.is_recursive = False
    ...
    def create_variables(self):
        ...
        """
        self.linear_matrix        = create_shared(self.hidden_size, self.hidden_size, name="Layer.linear_matrix")
    ...
    def activate(self, x):
        ...
        if x.ndim > 1:
            return self.activation(
                T.dot(self.linear_matrix, x.T)).T
        else:
            return self.activation(
                T.dot(self.linear_matrix, x))
...
class LSTM(RNN):
    ...
    def create_variables(self):
        ...
        self.in_gate     = Gate(self.hidden_size, T.nnet.sigmoid, self.clip_gradients)
        # forget gate for cells
        self.forget_gate = Gate(self.hidden_size, T.nnet.sigmoid, self.clip_gradients)
        ...
        # output modulation
        self.out_gate    = Gate(self.hidden_size, T.nnet.sigmoid, self.clip_gradients)
    ...
    def activate(self, x, h):
        ...
        in_gate = self.in_gate.activate(prev_h)

        # how much to forget the current contents of the memory
        forget_gate = self.forget_gate.activate(prev_h)
        ...
        out_gate = self.out_gate.activate(prev_h)
